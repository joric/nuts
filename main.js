...
function loadLevel(levelNumber) {
  ...
  updateMovesLog();
  renderGame();
  showMessage(`Level ${levelNumber} loaded! Click on a bolt to select a top nut.`, 'info');
  checkAndMarkCompletedBolts();
  updateSolutionDisplay();
}
...
function next() {
  ...
  if (makeMove(move.src, move.dst, true)) {
    updateSolutionDisplay();
    if (solutionMoves.length === 0) {
      showMessage('Solution completed! Press "Next Move" for the next level.', 'success');
    }
  } else {
